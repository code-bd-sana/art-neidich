...
import { ArrowDownToLine, ArrowRight, Download, Image as ImageIcon, Loader2 } from 'lucide-react';
      ...
      try {
        ...
        if (jobData?.reportId) {
          ...
          if (data.success) {
            ...
            const transformedPhotos = data.data.images?.map((imageGroup, index) => ({
              id: index + 1,
  ...
  if (photoData.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-800 mb-2">No Photos Available</h3>
    ...
    <div>
      ...
      <div className="hidden md:block border border-gray-200 rounded-lg overflow-hidden">
        ...
        <div className="divide-y divide-gray-200 bg-white">
          {photoData.map((item) => (
            <div key={item.id} className="grid grid-cols-12 gap-4 px-6 py-3 hover:bg-gray-50 transition-colors items-center">
              <div className="col-span-8">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-gray-900">{item.location}</span>
                </div>
      ...
      <div className="mt-6 bg-gray-50 rounded-lg p-4">
        <h3 className="text-sm font-medium text-gray-900 mb-3">Photo Summary</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-gray-900">{photoData.length}</p>
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-gray-900">
              {reportData?.createdAt ? new Date(reportData.createdAt).toLocaleDateString() : 'N/A'}
            </p>
      ...
      {reportData?.inspector && (
        <div className="mt-6 bg-gray-50 rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-900 mb-3">Inspector Information</h3>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-900">
                {reportData.inspector.firstName} {reportData.inspector.lastName}
              </p>
              <p className="text-xs text-gray-600">{reportData.inspector.email}</p>
            </div>
